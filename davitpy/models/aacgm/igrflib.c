...
int IGRF_loadcoeffs(char *filename)
{
  ...
  for (l=1; l<=IGRF_ORDER; l++) {
	...
	/* note, some files end lines with <CR><LF> while others are <LF> */
...
int IGRF_interpolate_coefs(void)
{
  ...
  fyear = ((igrf_date.dayno-1) +
	   (igrf_date.hour + (igrf_date.minute + igrf_date.second / 60.0)
	    / 60.0) / 24.0) / igrf_date.daysinyear + igrf_date.year;
...
int IGRF_SetDateTime(int year, int month, int day, int hour, int minute,
		     int second, char *filename)
{
	int err = 0;
  ...
  */
  ...
  if (code == 0)
      ...
      while (rr > eps)
	{
	  ...
	  if (fabs(rr) > 1e-16)
	    {
	      delt = 0.84 *pow(eps/rr,0.25);
	      /* this formula sucks because I have
	...
	; there is the suggestion that using the RK4 solution guarantees
