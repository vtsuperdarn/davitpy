...
double fyear = 0.0;		/* floating point year */

int myear_old = -1;
double fyear_old = -1.0;
...
int AACGM_v2_Rylm(double colat, double lon, int order, double *ylmval)
{
	...
	for (l=2; l<=order; l++)
	  {
	    ...
	    ylmval[ic] = (cos_theta * (2*l-1) * ylmval[ib] -
			  (l-1)*ylmval[ia]) / l;
	  }

	/*
	 * Generate P_l^l for l = 1 to (order+1)^2 using algorithm based upon
	 * (6.8.8) in Press et al., but incorporate longitude dependence,
	 * i.e., sin/cos (phi)
	 *
	...
	/*
	 * Generate P_l,l-1 to P_(order+1)^2,l-1 using algorithm based upon
	 *  (6.8.9)in Press et al., but incorporate longitude dependence, i.e.,
	 * sin/cos (phi)
	 *
	...
	/*
	 * Generate remaining P_l+2,m to P_(order+1)^2,m for each m = 1 to
	 * order-2 using algorithm based upon (6.8.7) in Press et al., but
	 * incorporate longitude dependence, i.e., sin/cos (phi).
	 *
	 * for each m value 1 to order-2 we have P_mm and P_m+1,m so we can
	 * compute P_m+2,m; P_m+3,m; etc.
	 *
	 */
	 ...
	 * 
	 * Note that this is NOT the standard spherical harmonic normalization
	 * factors.  The recursive algorithms above treat positive and negative
	 * values of m in the same manner. In order to use these algorithms the
	 * normalization must also be modified to reflect the symmetry.
	 * 
	 * Output values have been checked against those obtained using the
	 * internal IDL legendre() function to obtain the various associated 
	 * legendre polynomials.
	 * 
	 * As stated above, I think that this normalization may be unnecessary.
	 * The important thing is that the various spherical harmonics are
	 * orthogonal, rather than orthonormal.
	 * 
	 */
...
int convert_geo_coord(double lat_in, double lon_in, double height_in,
		      ...
		      char *igrf_filename)
{
  int i,j,k,l,m,flag; // AGB - removed a, t, f (no longer used)
  int i_err64, err;
...
//	extern int rylm();
  ...
  double lat_adj=0;
//	double lat_alt=0; AGB - SGS removed section that used this variable
  double colat_input; 
  ...
  /* COMMENT: SGS
   * 
   * This answers one of my questions about how the coordinates for AACGM
   * are guaranteed to be on the unit sphere. Here they compute xyz
   * indpendently using the SH coefficients for each coordinate. They reject
   * anything that is +/- .1 Re from the surface of the Earth.  They then scale 
   * each xyz coordinate by the computed radial distance.  This is a TERRIBLE
   * way to do things... but necessary for the inverse transformation.
   */
...
int AACGM_v2_LoadCoefFP(FILE *fp, int code)
{
  /*	char tmp[64]; */
  double tmp;
  int f,l,a,t; // AGB removing unused variable i;
...
int AACGM_v2_LoadCoefs(int year, char *root)
{
	...
	/* Create the name of the coefficient file */
	if (year <= 0) return -1;
		     ...
		     char *igrf_filename)
{
	...
	if (height < 0)
	  {
	    fprintf(stderr, "WARNING: coordinate transformations are not  "
		    "intended for altitudes < 0 km: %lf\n", height);
	    /*	return -2; */
	  }
	...
	if (fabs(in_lat) > 90.0)
	  {
	    fprintf(stderr, "ERROR: latitude must be in the range -90 to +90 "
		    "degrees: %lf\n", in_lat);
	    return -8;
	  }

	/* longitude out of bounds */
	/* SGS v2.3 removing requirement that longitude be -180 to 180. Does 
	 * not seems to matter and is inconsistent with IDL version: -180 to
	 * 180.
	...
	*/
	...
	/* if forward calculation (G2A) and input coordinates are given in
	    geodetic coordinates (default) then must first convert to
	    geocentric coordinates */
	if ((code & GEOCENTRIC) == 0 && (code & A2G) == 0)
	  {
	    geod2geoc(in_lat, in_lon, height, rtp);
	    /* modify lat/lon/alt to geocentric values */
	    in_lat = 90.0 - rtp[1] / DTOR;
	    in_lon = rtp[2] / DTOR;
	    height = (rtp[0] - 1.0) * RE;
	  }
...
int AACGM_v2_SetDateTime(int year, int month, int day, int hour, int minute,
			 int second, char *root)
{
  int err, doy, ndays;
  double fyear;

  doy = dayno(year, month, day, &ndays);
  fyear = year + ((doy - 1) + (hour + (minute + second / 60.0) / 60.0)
		  / 24.0) / ndays;
...
int AACGM_v2_GetDateTime(int *year, int *month, int *day, int *hour,
			 int *minute, int *second, int *dayno)
{
...
int AACGM_v2_SetNow(char *root)
{
  ...
  fyear = ((doy - 1) + (tm_now->tm_hour +
  			(tm_now->tm_min + tm_now->tm_sec / 60.0) / 60.0)
	   / 24.0) / ndays  + tm_now->tm_year;
  ...
  switch (ecode)
    ...
    case 0:	/* no Date/Time set */
      fprintf(stderr,
	      ...
	      "*                                                            *\n"
	      "* You must specifiy the date and time in order to use AACGM  *\n"
	      "* coordinates, which depend on the internal (IGRF) magnetic  *\n"
	      "* field. Before calling AACGM_v2_Convert() you must set the  *\n"
	      "* date and time to the integer values using the function:    *\n"
	      "*                                                            *\n"
	      ...
	      "*                                                            *\n"
	      "* subsequent calls to AACGM_v2_Convert() will use the last   *\n"
	      "* date and time that was set, so update to the actual date   *\n"
	      "* and time that is desired.                                  *"
	      "\n");
      break;
    ...
    case 1: /* Date/Time out of bounds */
      fprintf(stderr,
	      ...
	      "*                                                            *\n"
	      "* The current date range for AACGM-v2 coordinates is         *\n"
	      "* [1990-2020), which  corresponds to the date range for the  *\n"
	      "* IGRF12 model, including the 5-year secular variation.      *"
	      "\n");
      break;
    ...
    case 2: /* COEF Path not set */
      fprintf(stderr,
	      ...
	      "*                                                            *\n"
	      "* You must set the environment variable AACGM_v2_DAT_PREFIX  *\n"
	      "* or use the command line input.                             *"
	      "\n");
      break;
    }
  fprintf(stderr, "**************************************************************\n\n");
...
int AACGM_v2_TimeInterp(char *root)
{
  ...
  if (myear != myear_old)
    {
      /* load the new coefficients, if needed */
      err = AACGM_v2_LoadCoefs(myear, root);
      if (err != 0) return err;
  ...
  fyear =  ((aacgm_date.dayno - 1) +
	    (aacgm_date.hour + (aacgm_date.minute + aacgm_date.second
				/ 60.0) / 60.0) / 24.0)
    / aacgm_date.daysinyear + aacgm_date.year;
  ...
  if (fyear != fyear_old)
	...
	for (l=0;l<POLYORD;l++)
	  for (a=0;a<NCOORD;a++)
	    for (t=0;t<AACGM_KMAX;t++)
	      sph_harm_model.coef[t][a][l][f] = sph_harm_model.coefs[t][a][l][f][0] + (fyear - myear) * (sph_harm_model.coefs[t][a][l][f][1] - sph_harm_model.coefs[t][a][l][f][0])/5;

      height_old[0] = -1.0; /* force height interp because coeffs */
      height_old[1] = -1.0; /* have changed */
...
int AACGM_v2_Trace(double lat_in, double lon_in, double alt, double *lat_out,
		   double *lon_out, char *igrf_filename)
{
	int err, kk, idir;
	...
	rtp[1] = (90.0 - lat_in) * DTOR; /* colatitude in radians */
	...
	*/
	while (idir*xyzm[2] < 0.)
	  {
	    ...
	    AACGM_v2_RK45(xyzg, idir, &dsRE, eps, 1);
	    // set to 0 for RK4: /noadapt)
	...
	if (niter > 1)
	  {
	    /* now bisect fixed stepsize to land on magnetic equator w/in 1 m */
		...
		/* Is it possible that resetting here causes a doubling of */
		/* the tol? */
		if (idir * xyzm[2] > 0)
		  for (k=0;k<3;k++) xyzc[k] = xyzp[k];
		       ...
		       double *lat_out, double *lon_out, char *igrf_filename)
{
	...
	if (Lshell <(RE+alt)/RE)
	  ...
	  {
	    /* mag. Cartesian coordinates of fieldline trace starting point */
	    ...
	    if (niter > 1)
	      {
		/* now bisect fixed stepsize to land on mag equator w/in 1 m */
		for (k=0;k<3;k++) xyzc[k] = xyzp[k];
		kk = 0L;
		while (dsRE > 1e-3/RE) {
		  dsRE *= .5;
