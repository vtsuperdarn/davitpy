def readPygridRec(myFile,myGrid,sEpoch,eEpoch):
  ...
  **Args**:
    ...
    * **eEpoch**: read times < this time (in epoch).  if eEpoch <= sEpoch, only the first record after  sEpoch will be read
    **Returns**:
...
def writePygridRec(myFile,myGrid):
  ...
  if(myGrid.nVecs > 0):
    ...
    vecType = numpy.dtype([('index', 'i4'), ('lat', 'f4'), ('lon', 'f4'), ('v', 'f4'), ('w_l', 'f4'), \
                            ('p_l', 'f4'), ('stid', 'i2'), ('azm', 'f4'), ('bmnum', 'i2'), ('rng', 'i2')])
    ...
    for l in myGrid.lats:
      for c in l.cells:
        for i in range(0,c.nVecs):
          ...
          myVecs[cnt]['lat'] = c.center[0]
          myVecs[cnt]['lon'] = c.center[1]
          myVecs[cnt]['v'] = v.v
  ...
  if(myGrid.nAvg > 0):
    ...
    avgType = numpy.dtype([('index', 'i4'), ('lat', 'f4'), ('lon', 'f4'),  ('v', 'f4'), \
                            ('w_l', 'f4'), ('p_l', 'f4'), ('stid', 'i2'), ('azm', 'f4')])
    ...
    for l in myGrid.lats:
      for c in l.cells:
        for v in c.avgVecs:
          ...
          myAvg[cnt]['lat'] = c.center[0]
          myAvg[cnt]['lon'] = c.center[1]
          myAvg[cnt]['v'] = v.v
          myAvg[cnt]['w_l'] = v.w_l
          myAvg[cnt]['p_l'] = v.p_l
  ...
  if(myGrid.nMrg > 0):
    #create a numpy type to store the averaged vectors
    mrgType = numpy.dtype([('index', 'i4'), ('lat', 'f4'), ('lon', 'f4'), ('v', 'f4'), ('w_l', 'f4'), \
                            ('p_l', 'f4'), ('stid1', 'i2'), ('stid2', 'i2'),('azm', 'f4')])
    ...
    for l in myGrid.lats:
      for c in l.cells:
        if(c.mrgVec != None):
          ...
          myMrg[cnt]['lat'] = c.center[0]
          myMrg[cnt]['lon'] = c.center[1]
          myMrg[cnt]['v'] = v.v
          myMrg[cnt]['w_l'] = v.w_l
          myMrg[cnt]['p_l'] = v.p_l
...
def openPygrid(fileName,action):
  ...
  try: 
    ...
    print 'problem opening hdf5 file'
    print e
    return None
...
def locatePygridFile(dateStr,ext):
  ...
  fileName = radDir+'/'+dateStr+'.'+ext+'.pygrid.mlt.hdf5.bz2'
