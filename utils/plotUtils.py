    ...
    def __init__(self, datetime=None, coords='geo', projection='stere',
        ...
        **Example**:
        ...
        if self.lat_0 is None:
            self.lat_0 = 90.
            if boundinglat:
                self.lat_0 = math.copysign(self.lat_0, boundinglat)
...
def genCmap(param, scale, colors='lasse', lowGray=False):
    ...
    **Args**:
        * **param** (str): the parameter being plotted ('velocity' and
                           'phi0' are special cases, anything else gets
                           the same color scale)
        * **scale** (list): a list with the [min,max] values of the color
                            scale
        * **[colors]** (str): a string indicating which colorbar to use,
                              valid inputs are 'lasse', 'aj'.
                              default = 'lasse'
        * **[lowGray]** (boolean): a flag indicating whether to plot low
                                   velocities (|v| < 15 m/s) in gray.
                                   default = False
    **Returns**:
        * **cmap** (`matplotlib.colors.ListedColormap <http://matplotlib.org/api/colors_api.html?highlight=listedcolormap#matplotlib.colors.ListedColormap>`_):
                    the colormap generated.  This then gets passed to the
                    mpl plotting function (e.g. scatter, plot,
                    LineCollection, etc.)
        * **norm** (`matplotlib.colors.BoundaryNorm <http://matplotlib.org/api/colors_api.html?highlight=matplotlib.colors.boundarynorm#matplotlib.colors.BoundaryNorm>`_):
                    the colormap index.  This then gets passed to the mpl
                    plotting function (e.g. scatter, plot, LineCollection,
                    etc.)
        * **bounds** (list): the boundaries of each of the colormap
                             segments.  This can be used to manually
                             label the colorbar, for example.
    ...
    if (param == 'velocity'):
        ...
        if (colors == 'aj'):
            if (not lowGray):
                ...
                cmap = lcm([cmpr(.142), cmpr(.125), cmpr(.11), cmpr(.1),
                            cmpr(.175), cmpr(.158), cmj(.32), cmj(.37)])
            else:
                cmap = lcm([cmpr(.142), cmpr(.125), cmpr(.11), cmpr(.1),
                            '.6', cmpr(.175), cmpr(.158), cmj(.32),
                            cmj(.37)])
        else:
            if (not lowGray):
                #define our discrete colorbar
                cmap = lcm([cmj(.9), cmj(.8), cmj(.7), cmj(.65),
                            cmpr(.142), cmj(.45), cmj(.3), cmj(.1)])
            else:
                cmap = lcm([cmj(.9), cmj(.8), cmj(.7), cmj(.65), '.6',
                            cmpr(.142), cmj(.45), cmj(.3), cmj(.1)])
        ...
        if (colors == 'aj'):
            #define our discrete colorbar
            cmap = lcm([cmpr(.175), cmpr(.17), cmj(.32), cmj(.37),
                        cmpr(.142), cmpr(.13), cmpr(.11), cmpr(.10)])
        else:
            #define our discrete colorbar
            cmap = lcm([cmj(.1), cmj(.3), cmj(.45), cmpr(.142),
        ...
        if (colors == 'aj'):
            #define our discrete colorbar
            cmap = lcm([cmpr(.175), cmpr(.158), cmj(.32), cmj(.37),
                        cmpr(.142), cmpr(.13), cmpr(.11), cmpr(.10)])
        else:
            #define our discrete colorbar
            cmap = lcm([cmj(.1), cmj(.3), cmj(.45), cmpr(.142),
...
def drawCB(fig, coll, cmap, norm, map=False, pos=[0, 0, 1, 1]):
    """
    Manually draws a colorbar on a figure.  This can be used in lieu
    of the standard mpl colorbar function if you need the colorbar in
    a specific location.  See :func:`pydarn.plotting.rti.plotRti` for
    ...
    **Args**:
        ...
        * **[map]** (boolean): a flag indicating the we are drawing the
                               colorbar on a figure with a map plot
        * **[pos]** (list): The position of the colorbar with
    ...
    **Example**:
        ...
        cmap,norm,bounds = genCmap('velocity', [-200,200],
                                   colors='aj', lowGray=True)
...
def curvedEarthAxes(rect=111, fig=None,
    ...
    **Example**:
        ...
        Z = exp( -(r - 300 - ax.Re)**2 / 100**2 ) *
            np.cos(th[:, np.newaxis]/th.max()*4*np.pi)
    ...
    grid_helper = \
        floating_axes.GridHelperCurveLinear(tr, extremes=(0, angran,
                                                          Re + minalt,
                                                          Re + maxalt),
