...
class mapObj(basemap.Basemap):
    ...
    **Example**:
    ...
    def __init__(self, datetime=None, coords='geo', projection='stere',
        ...
        **Example**:
        ...
        if datetime is None and dateTime is None:
            estr = "{:s} WARNING: datetime/dateTime not specified,".format(rn)
            estr = "{:s} using current time.".format(estr)
            print estr
            datetime = dt.datetime.utcnow()
            dateTime = datetime
        elif datetime is None and dateTime is not None:
            estr = "{:s} WARNING: old syntax in use.  Setting ".format(rn)
            estr = "{:s}datetime to dateTime".format(estr)
            print estr
            datetime = dateTime
        elif datetime is not None and dateTime is None:
            estr = "{:s} ADVISEMENT: still testing for old syntax.".format(rn)
            print estr
            dateTime = datetime
        else:
            assert(datetime == dateTime),\
              "{:s} ERROR: Inconsistent datetimes were specified.".format(rn)
        ...
        assert(coords in self._coordsDict),\
          "{:s} ERROR: coords set to {},\n{}".format(rn, coords,
                                                     self.coords_string)

        # Add an extra member to the Basemap class.
        self.coords = coords

        # Set map projection limits and center point depending on hemisphere
        # selection
        ...
        if draw:
            self.draw(fill_alpha=fill_alpha)

    def draw(self, fill_alpha=0.5):
        import numpy as np
        ...
        _ = self.fillcontinents(color=self._fillContinents,
                                lake_color=self._fillLakes, alpha=fill_alpha)
        ...
        if self._grid:
            ...
            if self._gridLabels: 
                ...
                rotate_label = lablon - self.lon_0 if self.lat_0 >= 0 \
                               else self.lon_0 - lablon + 180.
                x,y = basemap.Basemap.__call__(self,
                                               lablon*np.ones(parallels.shape),
                                               parallels)
    ...
    def __call__(self, x, y, inverse=False, coords=None, altitude=0.):
        ...
        # If call was not from drawcoastlines, etc. then we do the conversion.
        # Otherwise, we do something different.
